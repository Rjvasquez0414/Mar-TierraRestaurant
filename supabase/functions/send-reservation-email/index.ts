...
serve(async (req) => {
  ...
  try {
    const data: ReservationEmail = await req.json();
    ...
    const customerRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `Reserva ${data.reservationCode} — Mar&Tierra Restaurant`,
    ...
    await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        to: [RESTAURANT_EMAIL],
        subject: `Nueva reserva: ${data.reservationCode} — ${data.customerName}`,
