-- =====================================================================
-- MIGRATION 012 — Reservas del mismo día: corregir zona horaria en la
--                 validación "la fecha debe ser hoy o posterior"
-- =====================================================================
-- create_reservation validaba 'p_date < CURRENT_DATE', y CURRENT_DATE se
-- evalúa en UTC. Colombia es UTC-5: después de las 7:00 p.m. hora local ya es
-- "mañana" en UTC, por lo que una reserva para HOY hecha de noche (ej. Vie/Sáb
-- con último turno 9:00 p.m.) se rechazaba con "La fecha debe ser hoy o
-- posterior". Se ancla a 'America/Bogota'.
--
-- Es la misma create_reservation canónica de migration-007 (aforo + valet),
-- con el único cambio en la línea de validación de fecha.
--
...
BEGIN
    ...
    END IF;
    -- "Hoy o posterior" en hora de Colombia (no UTC)
